...
impl M2Deployment {
    ...
    pub fn application_version(&self) -> PrimExpr<f64> {
        PrimExpr::new(self.shared().clone(), format!("{}.application_version", self.extract_ref()))
    }
    ...
    pub fn timeouts(&self) -> M2DeploymentTimeoutsElRef {
        M2DeploymentTimeoutsElRef::new(self.shared().clone(), format!("{}.timeouts", self.extract_ref()))
    }
}
