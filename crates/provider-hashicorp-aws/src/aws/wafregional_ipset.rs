...
impl WafregionalIpset {
    ...
    pub fn set_ip_set_descriptor(self, v: impl Into<BlockAssignable<WafregionalIpsetIpSetDescriptorEl>>) -> Self {
        match v.into() {
            BlockAssignable::Literal(v) => {
                self.0.data.borrow_mut().ip_set_descriptor = Some(v);
            },
            BlockAssignable::Dynamic(d) => {
                self.0.data.borrow_mut().dynamic.ip_set_descriptor = Some(d);
            },
        }
        self
    }
...
impl BuildWafregionalIpset {
    pub fn build(self, stack: &mut Stack) -> WafregionalIpset {
        let out = WafregionalIpset(Rc::new(WafregionalIpset_ {
            ...
            tf_id: self.tf_id,
            data: RefCell::new(WafregionalIpsetData {
                ...
                ip_set_descriptor: core::default::Default::default(),
