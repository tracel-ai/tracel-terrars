...
impl NetworkflowmonitorMonitor {
    ...
    pub fn set_local_resource(self, v: impl Into<BlockAssignable<NetworkflowmonitorMonitorLocalResourceEl>>) -> Self {
        match v.into() {
    ...
    pub fn set_remote_resource(self, v: impl Into<BlockAssignable<NetworkflowmonitorMonitorRemoteResourceEl>>) -> Self {
        match v.into() {
            BlockAssignable::Literal(v) => {
    ...
    pub fn monitor_arn(&self) -> PrimExpr<String> {
        PrimExpr::new(self.shared().clone(), format!("{}.monitor_arn", self.extract_ref()))
    }
