...
impl DataOdbNetworkPeeringConnections {
    ...
    pub fn odb_peering_connections(
        &self,
    ) -> ListRef<DataOdbNetworkPeeringConnectionsOdbPeeringConnectionsElRef> {
        ListRef::new(
            self.shared().clone(),
            format!("{}.odb_peering_connections", self.extract_ref()),
        )
    }
}
...
struct DataOdbNetworkPeeringConnectionsDynamic {
    odb_peering_connections:
        Option<DynamicBlock<DataOdbNetworkPeeringConnectionsOdbPeeringConnectionsEl>>,
}
