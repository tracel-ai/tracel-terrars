...
struct CognitoUserGroup_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<CognitoUserGroupData>,
}
#[derive(Clone)]
pub struct CognitoUserGroup(Rc<CognitoUserGroup_>);
impl CognitoUserGroup {
...
impl Referable for CognitoUserGroup {
...
impl Resource for CognitoUserGroup {}
impl ToListMappable for CognitoUserGroup {
    type O = ListRef<CognitoUserGroupRef>;
    fn do_map(self, base: String) -> Self::O {
        self.0.data.borrow_mut().for_each = Some(format!("${{{}}}", base));
        ListRef::new(self.0.shared.clone(), self.extract_ref())
    }
}
impl Resource_ for CognitoUserGroup_ {
...
pub struct BuildCognitoUserGroup {
    pub tf_id: String,
    #[doc = ""]
...
impl BuildCognitoUserGroup {
    pub fn build(self, stack: &mut Stack) -> CognitoUserGroup {
        let out = CognitoUserGroup(Rc::new(CognitoUserGroup_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(CognitoUserGroupData {
...
pub struct CognitoUserGroupRef {
    shared: StackShared,
    base: String,
}
impl Ref for CognitoUserGroupRef {
    fn new(shared: StackShared, base: String) -> Self {
        Self { shared, base }
    }
}
impl CognitoUserGroupRef {
    fn extract_ref(&self) -> String {
        self.base.clone()
    }
    fn shared(&self) -> &StackShared {
