...
struct KmsAliasData {
    ...
    target_key_id: PrimField<String>,
}
struct KmsAlias_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<KmsAliasData>,
}
#[derive(Clone)]
pub struct KmsAlias(Rc<KmsAlias_>);
impl KmsAlias {
    ...
    pub fn target_key_arn(&self) -> PrimExpr<String> {
        PrimExpr::new(
            self.shared().clone(),
            format!("{}.target_key_arn", self.extract_ref()),
        )
    }
    ...
    pub fn target_key_id(&self) -> PrimExpr<String> {
        PrimExpr::new(
            self.shared().clone(),
            format!("{}.target_key_id", self.extract_ref()),
        )
    }
}
impl Referable for KmsAlias {
...
impl ToListMappable for KmsAlias {
    type O = ListRef<KmsAliasRef>;
    fn do_map(self, base: String) -> Self::O {
        self.0.data.borrow_mut().for_each = Some(format!("${{{}}}", base));
        ListRef::new(self.0.shared.clone(), self.extract_ref())
    }
}
impl Resource_ for KmsAlias_ {
...
pub struct BuildKmsAlias {
    pub tf_id: String,
    #[doc = ""]
    pub target_key_id: PrimField<String>,
}
impl BuildKmsAlias {
    pub fn build(self, stack: &mut Stack) -> KmsAlias {
        let out = KmsAlias(Rc::new(KmsAlias_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(KmsAliasData {
...
pub struct KmsAliasRef {
    shared: StackShared,
    base: String,
}
impl Ref for KmsAliasRef {
