...
impl TimestreaminfluxdbDbCluster {
    ...
    #[doc = "Get a reference to the value of field `failover_mode` after provisioning.\nSpecifies the behavior of failure recovery when the primary node of the cluster\n\t\t\t\t\tfails."]
    pub fn failover_mode(&self) -> PrimExpr<String> {
        PrimExpr::new(
            self.shared().clone(),
            format!("{}.failover_mode", self.extract_ref()),
        )
    }
    ...
    pub fn log_delivery_configuration(
        &self,
    ) -> ListRef<TimestreaminfluxdbDbClusterLogDeliveryConfigurationElRef> {
        ListRef::new(
            self.shared().clone(),
...
pub struct BuildTimestreaminfluxdbDbCluster {
    ...
    #[doc = "The name that uniquely identifies the DB cluster when interacting with the \n\t\t\t\t\tAmazon Timestream for InfluxDB API and CLI commands. This name will also be a \n\t\t\t\t\tprefix included in the endpoint. DB cluster names must be unique per customer \n\t\t\t\t\tand per region."]
    ...
    #[doc = "A list of VPC subnet IDs to associate with the DB cluster. Provide at least \n\t\t\t\t\ttwo VPC subnet IDs in different availability zones when deploying with a Multi-AZ standby."]
...
impl ToListMappable for TimestreaminfluxdbDbClusterLogDeliveryConfigurationElS3ConfigurationEl {
    type O =
        BlockAssignable<TimestreaminfluxdbDbClusterLogDeliveryConfigurationElS3ConfigurationEl>;
    fn do_map(self, base: String) -> Self::O {
...
struct TimestreaminfluxdbDbClusterLogDeliveryConfigurationElDynamic {
    s3_configuration: Option<
        DynamicBlock<TimestreaminfluxdbDbClusterLogDeliveryConfigurationElS3ConfigurationEl>,
    >,
}
#[derive(Serialize)]
pub struct TimestreaminfluxdbDbClusterLogDeliveryConfigurationEl {
    #[serde(skip_serializing_if = "Option::is_none")]
    s3_configuration:
        Option<Vec<TimestreaminfluxdbDbClusterLogDeliveryConfigurationElS3ConfigurationEl>>,
...
impl TimestreaminfluxdbDbClusterLogDeliveryConfigurationElRef {
    ...
    pub fn s3_configuration(
        &self,
    ) -> ListRef<TimestreaminfluxdbDbClusterLogDeliveryConfigurationElS3ConfigurationElRef> {
        ListRef::new(
            self.shared().clone(),
...
struct TimestreaminfluxdbDbClusterDynamic {
    log_delivery_configuration:
        Option<DynamicBlock<TimestreaminfluxdbDbClusterLogDeliveryConfigurationEl>>,
}
