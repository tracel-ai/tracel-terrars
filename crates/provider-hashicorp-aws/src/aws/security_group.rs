...
impl SecurityGroup {
    ...
    pub fn ingress(&self) -> SetRef<SecurityGroupIngressElRef> {
        SetRef::new(
            self.shared().clone(),
            format!("{}.ingress", self.extract_ref()),
        )
    }
    ...
    pub fn revoke_rules_on_delete(&self) -> PrimExpr<bool> {
        PrimExpr::new(
            self.shared().clone(),
            format!("{}.revoke_rules_on_delete", self.extract_ref()),
        )
    }
    ...
    pub fn timeouts(&self) -> SecurityGroupTimeoutsElRef {
        SecurityGroupTimeoutsElRef::new(
            self.shared().clone(),
...
impl Referable for SecurityGroup {
...
impl SecurityGroupEgressElRef {
    ...
    pub fn ipv6_cidr_blocks(&self) -> ListRef<PrimExpr<String>> {
        ListRef::new(
            self.shared().clone(),
            format!("{}.ipv6_cidr_blocks", self.base),
        )
    }
