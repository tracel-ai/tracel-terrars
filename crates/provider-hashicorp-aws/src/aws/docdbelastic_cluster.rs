...
impl DocdbelasticCluster {
    ...
    pub fn admin_user_name(&self) -> PrimExpr<String> {
        PrimExpr::new(self.shared().clone(), format!("{}.admin_user_name", self.extract_ref()))
    }
    ...
    pub fn shard_capacity(&self) -> PrimExpr<f64> {
        PrimExpr::new(self.shared().clone(), format!("{}.shard_capacity", self.extract_ref()))
    }
    ...
    pub fn timeouts(&self) -> DocdbelasticClusterTimeoutsElRef {
        DocdbelasticClusterTimeoutsElRef::new(self.shared().clone(), format!("{}.timeouts", self.extract_ref()))
    }
}
