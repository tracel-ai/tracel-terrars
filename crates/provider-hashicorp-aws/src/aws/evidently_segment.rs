...
struct EvidentlySegment_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<EvidentlySegmentData>,
}
#[derive(Clone)]
pub struct EvidentlySegment(Rc<EvidentlySegment_>);
impl EvidentlySegment {
...
impl Referable for EvidentlySegment {
...
impl Resource for EvidentlySegment {}
impl ToListMappable for EvidentlySegment {
    type O = ListRef<EvidentlySegmentRef>;
    fn do_map(self, base: String) -> Self::O {
        self.0.data.borrow_mut().for_each = Some(format!("${{{}}}", base));
        ListRef::new(self.0.shared.clone(), self.extract_ref())
    }
}
impl Resource_ for EvidentlySegment_ {
    fn extract_resource_type(&self) -> String {
        "aws_evidently_segment".into()
    }
...
pub struct BuildEvidentlySegment {
    pub tf_id: String,
    #[doc = ""]
...
impl BuildEvidentlySegment {
    pub fn build(self, stack: &mut Stack) -> EvidentlySegment {
        let out = EvidentlySegment(Rc::new(EvidentlySegment_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(EvidentlySegmentData {
...
pub struct EvidentlySegmentRef {
    shared: StackShared,
    base: String,
}
impl Ref for EvidentlySegmentRef {
    fn new(shared: StackShared, base: String) -> Self {
        Self { shared, base }
    }
}
impl EvidentlySegmentRef {
    fn extract_ref(&self) -> String {
        self.base.clone()
    }
    fn shared(&self) -> &StackShared {
