...
struct EksAccessPolicyAssociationData {
    ...
    #[serde(skip_serializing_if = "Option::is_none")]
    access_scope: Option<Vec<EksAccessPolicyAssociationAccessScopeEl>>,
    #[serde(skip_serializing_if = "Option::is_none")]
...
impl EksAccessPolicyAssociation {
    ...
    pub fn set_access_scope(self, v: impl Into<BlockAssignable<EksAccessPolicyAssociationAccessScopeEl>>) -> Self {
        match v.into() {
            BlockAssignable::Literal(v) => {
                self.0.data.borrow_mut().access_scope = Some(v);
            },
            BlockAssignable::Dynamic(d) => {
                self.0.data.borrow_mut().dynamic.access_scope = Some(d);
            },
        }
        self
    }
    ...
    pub fn associated_at(&self) -> PrimExpr<String> {
        PrimExpr::new(self.shared().clone(), format!("{}.associated_at", self.extract_ref()))
    }
    ...
    pub fn access_scope(&self) -> ListRef<EksAccessPolicyAssociationAccessScopeElRef> {
        ListRef::new(self.shared().clone(), format!("{}.access_scope", self.extract_ref()))
    }
...
impl BuildEksAccessPolicyAssociation {
    pub fn build(self, stack: &mut Stack) -> EksAccessPolicyAssociation {
        let out = EksAccessPolicyAssociation(Rc::new(EksAccessPolicyAssociation_ {
            ...
            tf_id: self.tf_id,
            data: RefCell::new(EksAccessPolicyAssociationData {
                ...
                access_scope: core::default::Default::default(),
...
pub struct EksAccessPolicyAssociationAccessScopeEl {
    #[serde(skip_serializing_if = "Option::is_none")]
    namespaces: Option<SetField<PrimField<String>>>,
    #[serde(rename = "type")]
