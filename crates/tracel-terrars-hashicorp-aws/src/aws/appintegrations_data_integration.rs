...
impl AppintegrationsDataIntegration {
    ...
    pub fn set_schedule_config(
        self,
        v: impl Into<BlockAssignable<AppintegrationsDataIntegrationScheduleConfigEl>>,
    ) -> Self {
        match v.into() {
            BlockAssignable::Literal(v) => {
                self.0.data.borrow_mut().schedule_config = Some(v);
            },
            BlockAssignable::Dynamic(d) => {
                self.0.data.borrow_mut().dynamic.schedule_config = Some(d);
            },
        }
        self
    }
    ...
    pub fn source_uri(&self) -> PrimExpr<String> {
        PrimExpr::new(self.shared().clone(), format!("{}.source_uri", self.extract_ref()))
    }
    ...
    pub fn schedule_config(&self) -> ListRef<AppintegrationsDataIntegrationScheduleConfigElRef> {
        ListRef::new(self.shared().clone(), format!("{}.schedule_config", self.extract_ref()))
    }
}
...
impl BuildAppintegrationsDataIntegration {
    pub fn build(self, stack: &mut Stack) -> AppintegrationsDataIntegration {
        let out = AppintegrationsDataIntegration(Rc::new(AppintegrationsDataIntegration_ {
            ...
            tf_id: self.tf_id,
            data: RefCell::new(AppintegrationsDataIntegrationData {
