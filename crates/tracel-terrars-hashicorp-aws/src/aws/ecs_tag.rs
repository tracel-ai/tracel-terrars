...
struct EcsTag_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<EcsTagData>,
}

#[derive(Clone)]
pub struct EcsTag(Rc<EcsTag_>);
...
impl Referable for EcsTag {
    fn extract_ref(&self) -> String {
        format!("{}.{}", self.0.extract_resource_type(), self.0.extract_tf_id())
    }
}

impl Resource for EcsTag { }

impl ToListMappable for EcsTag {
    type O = ListRef<EcsTagRef>;
...
impl BuildEcsTag {
    pub fn build(self, stack: &mut Stack) -> EcsTag {
        let out = EcsTag(Rc::new(EcsTag_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(EcsTagData {
...
pub struct EcsTagRef {
    shared: StackShared,
    base: String,
}

impl Ref for EcsTagRef {
