...
struct IamUser_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<IamUserData>,
}

#[derive(Clone)]
pub struct IamUser(Rc<IamUser_>);
...
impl IamUser {
    ...
    #[doc =
        "Get a reference to the value of field `force_destroy` after provisioning.\nDelete user even if it has non-Terraform-managed IAM access keys, login profile or MFA devices"]
...
impl ToListMappable for IamUser {
    type O = ListRef<IamUserRef>;
...
impl BuildIamUser {
    pub fn build(self, stack: &mut Stack) -> IamUser {
        let out = IamUser(Rc::new(IamUser_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(IamUserData {
...
pub struct IamUserRef {
    shared: StackShared,
    base: String,
}

impl Ref for IamUserRef {
