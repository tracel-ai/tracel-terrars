...
struct IamGroup_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<IamGroupData>,
}

#[derive(Clone)]
pub struct IamGroup(Rc<IamGroup_>);
...
impl ToListMappable for IamGroup {
    type O = ListRef<IamGroupRef>;
...
impl BuildIamGroup {
    pub fn build(self, stack: &mut Stack) -> IamGroup {
        let out = IamGroup(Rc::new(IamGroup_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(IamGroupData {
...
pub struct IamGroupRef {
    shared: StackShared,
    base: String,
}

impl Ref for IamGroupRef {
