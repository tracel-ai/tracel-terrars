...
struct IamPolicy_ {
    shared: StackShared,
    tf_id: String,
    data: RefCell<IamPolicyData>,
}

#[derive(Clone)]
pub struct IamPolicy(Rc<IamPolicy_>);
...
impl IamPolicy {
    ...
    #[doc = "Get a reference to the value of field `attachment_count` after provisioning.\n"]
    pub fn attachment_count(&self) -> PrimExpr<f64> {
        PrimExpr::new(self.shared().clone(), format!("{}.attachment_count", self.extract_ref()))
    }
...
impl ToListMappable for IamPolicy {
    type O = ListRef<IamPolicyRef>;
...
impl BuildIamPolicy {
    pub fn build(self, stack: &mut Stack) -> IamPolicy {
        let out = IamPolicy(Rc::new(IamPolicy_ {
            shared: stack.shared.clone(),
            tf_id: self.tf_id,
            data: RefCell::new(IamPolicyData {
...
pub struct IamPolicyRef {
    shared: StackShared,
    base: String,
}

impl Ref for IamPolicyRef {
