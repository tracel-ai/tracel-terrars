use super::sourceschema::{
    AggCollType, AggCollTypeKey, AggObjType, Block, NestedBlock, NestingMode, ScalarTypeKey, Value,
    ValueBehaviorHelper, ValueSchema, ValueSchemaNested,
};
...
pub fn generate_simple_type(
    ...
    (
        quote!(PrimField < #raw >),
        Some((quote!(PrimExpr), quote!(PrimExpr < #raw >))),
    )
}

pub fn add_path(v: &[String], e: &str) -> Vec<String> {
...
pub fn sanitize(v: &str) -> (bool, String) {
    match v {
        ...
        | "priv" | "typeof" | "unsized" | "virtual" | "yield" | "try" => (true, format!("{}_", v)),
        s => (false, s.into()),
    }
}
...
pub fn generate_field(
    ...
    match behavior {
        ValueBehaviorHelper::UserRequired => {
            out.builder_fields
                .push(quote!(#[doc = #field_doc] pub #field_name: #rust_field_type));
            out.copy_builder_fields
                .push(quote!(#field_name: self.#field_name));
            if sanitized {
                out.fields
                    .push(quote!(#[serde(rename = #k)] #field_name: #rust_field_type));
            } else {
        ...
        ValueBehaviorHelper::UserOptional | ValueBehaviorHelper::UserOptionalComputed => {
            ...
            if let Some(block_type) = block {
                out.dynamic_block_fields
                    .push(quote!(#field_name: Option < DynamicBlock < #block_type >>));
    ...
    if let Some((t1, t2)) = rust_field_ref_type {
        if self_has_identity {
            ...
            out.ref_methods
                .push(quote!(#[doc = #ref_doc] pub fn #field_name(&self) -> #t2 {
...
fn generate_type(
    ...
    match at {
        ...
        (Some(ValueSchema::AggColl(at)), None) => {
            generate_agg_type_coll(extra_types, path, at.as_ref())
        }
        (Some(ValueSchema::AggObj(at)), None) => {
            generate_agg_type_obj(extra_types, path, at.as_ref())
        }
...
pub fn generate_fields_from_value_map(
    ...
    for (k, v) in fields {
        let mut path = path.to_owned();
        path.extend(k.split("_").map(ToString::to_string));
        let (rust_field_type, rust_field_ref_type) = generate_type(
            &mut out.extra_types,
            &path,
            (v.r#type.as_ref(), v.nested_type.as_ref()),
        );
        generate_field(
            out,
            k,
...
pub fn generate_fields_from_valueschema_map(
    ...
    for (k, v) in fields {
        ...
        let (rust_field_type, rust_field_ref_type) =
            generate_type(&mut out.extra_types, &path, (Some(v), None));
        generate_field(
            out,
            k,
            rust_field_type,
